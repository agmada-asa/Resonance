...
class AudioActionProcessor:
    def __init__(self, config: AudioConfig = DEFAULT_CONFIG):
        self.config = config

    def encode_action(self, action, parameter):
        if action == 'no_action':
    ...
    def apply_action(self, audio, action, parameter):
        ...
        """
        if action == 'no_action':
            return audio, self.encode_action(action, parameter)
        elif action == 'gain':
            ...
            return self.gain(audio, db), self.encode_action(action, db)
        elif action == 'pitch_change':
            ...
            return self.change_pitch(audio, semitones), self.encode_action(action, semitones)
        elif action == 'low_pass':
            return self.low_pass(audio, parameter), self.encode_action(action, parameter)
        elif action == 'high_pass':
            return self.high_pass(audio, parameter), self.encode_action(action, parameter)
        else:
            raise ValueError("Unsupported action type. Use 'no_action', 'gain', 'pitch_change', 'low_pass', or 'high_pass'.")

    def gain(self, audio, gain_db):
        """
    ...
    def change_pitch(self, audio, semitones):
        """
        ...
        :return: Numpy array containing the modified audio samples
        """
        return librosa.effects.pitch_shift(audio, sr=self.config.sample_rate, n_steps=semitones, scale=True) # scale=True ensures that the duration and volume of the audio remains the same after pitch shifting

    def low_pass(self, audio, cutoff):
        nyquist = 0.5 * self.config.sample_rate

        if not 20 <= cutoff <= nyquist:
            raise ValueError("Cutoff frequency is out of bounds.")

        sos = butter(N=5, Wn=cutoff, fs=self.config.sample_rate, btype='low', analog=False, output='sos')
        return sosfilt(sos, audio)

    def high_pass(self, audio, cutoff):
        nyquist = 0.5 * self.config.sample_rate

        if not 20 <= cutoff <= nyquist:
            raise ValueError("Cutoff frequency is out of bounds.")

        sos = butter(N=5, Wn=cutoff, fs=self.config.sample_rate, btype='high', analog=False, output='sos')
        return sosfilt(sos, audio)


DEFAULT_ACTION_PROCESSOR = AudioActionProcessor()


def encode_action(action, parameter):
    return DEFAULT_ACTION_PROCESSOR.encode_action(action, parameter)


def apply_action(audio, action, parameter):
    return DEFAULT_ACTION_PROCESSOR.apply_action(audio, action, parameter)


def gain(audio, gain_db):
    return DEFAULT_ACTION_PROCESSOR.gain(audio, gain_db)


def change_pitch(audio, semitones):
    return DEFAULT_ACTION_PROCESSOR.change_pitch(audio, semitones)


def low_pass(audio, cutoff):
    return DEFAULT_ACTION_PROCESSOR.low_pass(audio, cutoff)


def high_pass(audio, cutoff):
    return DEFAULT_ACTION_PROCESSOR.high_pass(audio, cutoff)
