...
def generate_training_data(seed=42):
    ...
    np.savez_compressed(
        output_dir / "train_pitch_only.npz",
    ...
    np.savez_compressed(
        output_dir / "test_pitch_only.npz",
    ...
    with open(output_dir / "metadata_train_pitch_only.jsonl", "w") as f:
        for entry in train_metadata: 
            f.write(json.dumps(entry) + "\n")

    with open(output_dir / "metadata_val_pitch_only.jsonl", "w") as f:
        for entry in val_metadata: 
            f.write(json.dumps(entry) + "\n")

    with open(output_dir / "metadata_test_pitch_only.jsonl", "w") as f:
